...
void outGoingChat(){
	...
	cout <<easyComparisonString.length() << endl;
	while(easyComparisonString.length() > 500){
	    //cout <<easyComparisonString.length() << endl;
		//prompt to reenter a message
	    //clearning the string in order to use it for the next message
		easyComparisonString.clear();
		//memset(&outBuffer, 0, sizeof(outBuffer));
		cout << "ERROR: Your message is more than 500 characters long." << endl;
		//prompt to reenter a message
		cout << serverHandle << "> ";
		cin.getline(outBuffer, 500);
		//easyComparisonString.clear();
