...
class BoxOfProduce{
    ...
    //REQUIREMENT #15: Demonstrates at least one string variable (c-style string),
        BoxOfProduce(char *selection1, char*selection2, char *selection3){
        //selection1 = &produce1;
    ...
    void Display(void);
    void Replace (int produce, char *selection);
};
...
int main(int argc, char* argv[])
{
    ...
    else if(argc==6)
    {
        ...
        reviews[1][entry-1]=atoi(argv[entry]);//loops though the entries, need to changed entries to atoi per http://en.cppreference.com/w/cpp/string/byte/atoi
        }
        NUM_REVIEWERS = 1;
...
void BoxOfProduce::Display(void){
    //BoxOfProduce produce;
    //BoxOfProduce*pointer;
    //pointer = &produce;
...
void getFlowerOrder(menuFlower menu[],menuFlower order[],int& items){
    ...
    while(toupper(yesno)=='Y'){
        ...
        //while(yesno != 'Y' || yesno != 'N'||yesno != 'y' || yesno != 'n'){
            //cout << "Error,invalid entry. Please type Y for yes and N for no" << endl;
            //cout<<"Would you like another item?(y/n)? ";
           // cin >> yesno;
           //break;
        }
